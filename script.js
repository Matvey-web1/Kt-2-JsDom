...
mainDiv.style.cssText = "display: flex; justify-content: space-between; flex-wrap: wrap;"
...
for (cat of cats) {
    ...
    mainDiv.append(div)
    div.style.cssText = "height: 500px; width: 500px; border: 4px solid black; display: flex; flex-direction: column; align-items: center; background-color: rgb(235, 191, 115); margin: 20px"
    //Картинка в div'e
    ...
    //отрисовка имени и возраста

    const div2 = document.createElement("div")
    div.append(div2)

    div2.style.cssText = "height: 72px; width: 300px; display: flex; align-items: center; justify-content: space-around;"

    const p1 = document.createElement("p")
    p1.textContent = `${cat.name}` //Разделить с помощью флекс бокса
    div2.append(p1)
    ...
    for (let star = 1; star <= cat.rate; star++){
        h3.textContent += '★'
    }

    h3.textContent += ` ${cat.rate}/10`
    //описание
    ...
    description.style.padding = "0px 10px"
